...
var mainScene, uniMesh, stripesMeshes, mainSceneShadow, cylinders, workingGroupSprites;
var stripes = [];
var xPositions = [];
var stripesColor = [];
var hovered = [];
...
function initMainScene() {
    ...
    //initialise and add main scene
    mainScene = new THREE.Scene();
    scene.add(mainScene);

    //initialise fog so that objects far away slowly dissapear
    scene.fog = new THREE.Fog(0x123c5c, 60, 2250);

    //initialise object that later holds all stripes of the logo
    stripesMeshes = new THREE.Object3D();

    //initialise the 3 point light
    add3PointLight(0, 0, mainScene);

    //University Logo init function
    initUniMesh();
...
function initUniMesh() {
    ...
    loader.load("models/uni.json", function(geometry) {
        ...
        //initialise shadow
        var geometry = new THREE.PlaneGeometry(190, 85);
    ...
    //then initialise the about scene in front of the main menu. 
    initAbout();
}
...
function initAbout() {
    ...
    geometry = new THREE.CylinderBufferGeometry(60, 60, 33, 32);
    var cylinder2 = new THREE.Mesh(geometry, material);
    cylinder2.position.set(0, 40.5, 900);

    geometry = new THREE.CylinderBufferGeometry(60, 60, 24, 32);
    var cylinder3 = new THREE.Mesh(geometry, material);
    cylinder3.position.set(0, 72.5, 900);

    geometry = new THREE.CylinderBufferGeometry(60, 60, 22.5, 32);
    var cylinder4 = new THREE.Mesh(geometry, material);
    cylinder4.position.set(0, 99.5, 900);

    geometry = new THREE.CylinderBufferGeometry(60, 60, 18, 32);
...
function loadAboutSprites() {
    ...
    workingGroupSprites = new THREE.Object3D();
    workingGroupSprites.position.set(450, 0, 750);
    mainScene.add(workingGroupSprites);
    ...
    loader.load('images/informatik.png', function(texture) {
        ...
        sprite.userData = { URL: "https://www.uni-koblenz-landau.de/de/koblenz/fb4/ifi" };
        sprite.scale.set(125, 62.5, 1.0);
        sprite.position.set(-200, 0, 30);
        workingGroupSprites.add(sprite);
    });

    loader.load('images/west.png', function(texture) {
        var spriteMaterial = new THREE.SpriteMaterial({ map: texture });
        var sprite = new THREE.Sprite(spriteMaterial);
        sprite.userData = { URL: "https://west.uni-koblenz.de/" };
    ...
    loader.load('images/ist.png', function(texture) {
        var spriteMaterial = new THREE.SpriteMaterial({ map: texture });
        var sprite = new THREE.Sprite(spriteMaterial);
        sprite.userData = { URL: "https://www.uni-koblenz-landau.de/de/koblenz/fb4/ist" };
    ...
    loader.load('images/agas.png', function(texture) {
        var spriteMaterial = new THREE.SpriteMaterial({ map: texture });
        var sprite = new THREE.Sprite(spriteMaterial);
        sprite.userData = { URL: "https://www.uni-koblenz-landau.de/de/koblenz/fb4/icv/agpaulus" };
        sprite.scale.set(155.25, 75, 1.0);
        sprite.position.set(0, 100, 40);

        workingGroupSprites.add(sprite);
    });
    cgSprite = genBigSprite('COMPUTERGRAFIK', -120, -200, -30, "rgba(255,255,255, 1.0)");
    cgSprite.userData = { URL: "https://www.uni-koblenz-landau.de/de/koblenz/fb4/icv/agmueller" };
    workingGroupSprites.add(cgSprite);
    clSprite = genBigSprite('COMPUTERLINGUISTIK', 80, -200, -30, "rgba(255,255,255, 1.0)");
    clSprite.userData = { URL: "https://www.uni-koblenz-landau.de/de/koblenz/fb4/icv/agharbusch" };
    workingGroupSprites.add(clSprite);
}
//function that generates a white Sprite
...
//function that generates the sprites for the percentages
...
//function that generates the headline sprites
...
function clickableWorkingGroupSprites() {
    var intersects = raycaster.intersectObjects(workingGroupSprites.children);
    if (intersects.length > 0) {
