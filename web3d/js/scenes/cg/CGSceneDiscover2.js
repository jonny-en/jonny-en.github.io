function cgSceneDiscover2() {
	...
	$('#content').load("content/cg/cgSceneDiscover2.html", function() {
		...
		var bumpScale = cgSceneDiscover2Gui.add(parameters2, 'bumpScale', 0, 4).listen().name('Bump-Effekt');
		var shininess = cgSceneDiscover2Gui.add(parameters2, 'shininess', 0, 20).listen().name('Schein-Effekt');
		var cloudsOpacity = cgSceneDiscover2Gui.add(parameters2, 'cloudsOpacity', 0, 1).listen().name('Deckkraft Wolken');
		var glowOpacity = cgSceneDiscover2Gui.add(parameters2, 'glowOpacity', 0, 1).listen().name('Deckkraft Atmosph&auml;re');
		cgSceneDiscover2Gui.add(parameters2, 'reset').name("Parameter zur&uuml;cksetzen");
		var nightMode = cgSceneDiscover2Gui.add(parameters2, 'nightMode').listen().name('NACHT MODUS')
